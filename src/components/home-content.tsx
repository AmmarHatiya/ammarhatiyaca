...
import type { MDXFile } from "@/lib/content";
import type { ProjectFrontmatter } from "@/lib/content";

interface Props {
  skills: Record<string, string[]>;
  featuredProjects: MDXFile<ProjectFrontmatter>[];
}

export function HomeContent({ skills, featuredProjects }: Props) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 sm:px-6">
      ...
      <section
        ...
        className={`flex flex-col items-start justify-center py-20 sm:py-28 transition-all duration-200 ease-out ${
      ...
      <section
        ...
        className={`pb-16 transition-all duration-200 ease-out ${
          mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        }`}
      ...
      <section
        aria-label="Connect"
        className={`flex flex-col items-center gap-4 border-t border-border/40 py-16 text-center transition-all duration-200 ease-out ${
          mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        }`}
